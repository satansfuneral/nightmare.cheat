...
void CCSGrenadeHint::View(CViewSetup* setup)
{
	...
	if (local && local->IsAlive())
	{
		...
		CCSGrenade* csGrenade = (CCSGrenade*)weapon;
...
{
	Color TracerColor = Color(g_Options.Colors.GrenadeLine);
	Color boxColor = Color(g_Options.Colors.GrenadeBounce);
	...
	if ((type) && path.size()>1)
	{
		...
		for (auto it = path.begin(), end = path.end(); it != end; ++it)
		{
			...
			{
				g_Surface->DrawSetColor(TracerColor);
