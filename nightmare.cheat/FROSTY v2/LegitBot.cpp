...
void legitbot::do_aimbot(C_BaseEntity *local, CBaseCombatWeapon *weapon, CInput::CUserCmd *cmd)
{
	...
	if (g_Options.LegitBot.RCSFix)
		if ((recoil_min >= 1.f || recoil_max >= 1.f) && Globals::Shots >= 3)
			final_angle = view_angle - delta_angle / 1 * randomSmoothing;
		else
...
void legitbot::weapon_settings(CBaseCombatWeapon* weapon)
{
	...
	else if (MiscFunctions::IsPistol(weapon) || MiscFunctions::IsRevolver(weapon))
	{
